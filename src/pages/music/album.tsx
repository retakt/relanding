...
export default function AlbumPage() {
  ...
  const handlePlayAlbum = () => {
    ...
    isAlbumPlaying ? pause() : play(toPlayerTrack(playable[0]), playable.map(toPlayerTrack));
  };
  ...
  return (
    <div className="w-full max-w-3xl space-y-4">
      ...
      {loading ? (
        ...
        <>
          {/* ── Album header card ── */}
          <div className={`rounded-xl border ${palette.border} bg-gradient-to-br ${palette.headerGradient} px-4 py-4 shadow-sm`}>

            {/* Admin edit — top right, only for admins */}
            {isAdmin && (
            ...
            {/* Main row: cover+play (left) · details (right) */}
            <div className="flex gap-4 items-start">

              {/* LEFT: Cover + play button directly below */}
              <div className="shrink-0 flex flex-col items-center gap-2">
                {/* Cover — compact, consistent size */}
                ...
                {/* Play button — directly below cover, icon only */}
                {hasAnyAudio && (
                  <button
                    onClick={handlePlayAlbum}
                    className={`w-8 h-8 flex items-center justify-center rounded-full transition-all hover:scale-110 active:scale-95 shadow-sm ${palette.iconBg} ${palette.iconColor} border ${palette.border}`}
              ...
              <div className="flex-1 min-w-0 space-y-1.5">
                ...
                <div className="flex flex-wrap items-center gap-1.5 text-[11px] text-muted-foreground/70">
                  {albumInfo?.year && <span className="tabular-nums">{albumInfo.year}</span>}
                  {albumInfo?.year && <span className="opacity-40">·</span>}
                  <span>{tracks.length} {tracks.length === 1 ? "track" : "tracks"}</span>
                  {albumInfo?.genre && (
                    <><span className="opacity-40">·</span><span>{albumInfo.genre}</span></>
                  )}
                </div>
                ...
                {albumInfo?.tags && albumInfo.tags.length > 0 && (
                  <div className="flex flex-wrap gap-1 pt-0.5">
                    {albumInfo.tags.map((tag) => (
                      <span key={tag} className={`text-[9px] px-1.5 py-0.5 rounded-full font-medium ${palette.badge}`}>
                        #{tag}
                      </span>
                    ))}
                  </div>
                ...
                {/* Description — only if it exists, no separator needed */}
